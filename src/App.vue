<template>
  <div
    class="min-h-screen w-[1200px] flex flex-col justify-center items-center bg-gradient-to-br from-cyan-300 to-sky-300 mx-auto"
  >
    <div class="bg-white shadow-lg rounded-lg p-4 border w-4/5">
      ...
      <div
        v-if="items.length === 0"
        class="bg-purple-300 px-4 py-2 rounded-lg text-center mb-6"
      >
...
const addNewList = () => {
  if (inputList.value.trim()) {
    items.value.push({ text: inputList.value, completed: false })
    inputList.value = ''
  }
}
